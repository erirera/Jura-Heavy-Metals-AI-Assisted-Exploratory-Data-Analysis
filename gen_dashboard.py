...
csv_path = os.path.join(BASE, "jura_prediction_set.csv")
...
with open(csv_path, newline="") as f:
    ...
    for line in reader:
        ...
        try:
            rows.append({
                "id": int(line[0]), "x": float(line[1]), "y": float(line[2]),
                "rock": int(line[3]), "land": int(line[4]),
                "Cd": float(line[5]), "Cu": float(line[6]), "Pb": float(line[7]),
                "Co": float(line[8]), "Cr": float(line[9]),
                "Ni": float(line[10]), "Zn": float(line[11]),
            })
        except:
            pass
...
<title>Jura EDA Dashboard — Prediction Set (n=259)</title>
...
<header>
  ...
  <div class="header-text">
    ...
    <p>Prediction Set &middot; Swiss Jura Region &middot; Spatial Geochemical Survey</p>
  </div>
  <div class="badge">n = $n_samples samples</div>
  <a href="../Validation set/index2.html" class="nav-btn">Validation Set &rarr;</a>
</header>
<main>
...
<footer>Jura EDA Dashboard &middot; Swiss Jura Heavy Metals Dataset &middot; Prediction Set (n=259)</footer>
<script>
var ROWS = $rows_js;
var STATS = $stats_js;
var CORR = $corr_js;
